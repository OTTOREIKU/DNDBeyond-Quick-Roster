...
// @match        https://advanced-sheets.production.roll20preflight.net/*
// @grant        GM_setValue
// @grant        GM_getValue
...
(function() {
    ...
    // --- DETECT MODE: POPOUT SCRAPER vs MAIN TOOL ---
    const currentUrl = window.location.href;
    const isSheetUrl = currentUrl.includes('popout') || currentUrl.includes('character') || window.location.hostname.includes('roll20preflight');
    ...
    // =================================================================
    // POPOUT MODE (SCRAPER ONLY)
    // =================================================================
    function initScraper() {
        const hideStyle = document.createElement('style');
        hideStyle.innerHTML = `#ddb-master-container, #ddb-sheet-viewer { display: none !important; }`;
        ...
        panel.innerHTML = `
            <div style="background:#111; border: 1px solid #d51a88; padding:4px 8px; border-radius:6px; display:flex; gap:8px; align-items:center; box-shadow:0 4px 10px rgba(0,0,0,0.8); white-space:nowrap;">
                <span id="cache-count" style="color:#ff80cc; font-size:11px; font-weight:bold; font-family:sans-serif;">Views: 0</span>
                <button id="btn-cache-view" style="background:#333; color:white; border:1px solid #555; padding:4px 8px; font-size:11px; border-radius:4px; cursor:pointer; font-weight:bold; font-family:sans-serif; white-space:nowrap; transition:0.2s;">+ Capture View</button>
                <button id="btn-save-roster" style="background:#d51a88; color:white; border:none; padding:4px 8px; font-size:11px; border-radius:4px; cursor:pointer; font-weight:bold; font-family:sans-serif; white-space:nowrap; transition:0.2s;">Save & Close</button>
            </div>
        `;
        document.body.appendChild(panel);

        document.getElementById('btn-cache-view').onmouseover = (e) => e.target.style.background = '#444';
        document.getElementById('btn-cache-view').onmouseout = (e) => e.target.style.background = '#333';
        document.getElementById('btn-save-roster').onmouseover = (e) => e.target.style.background = '#e62497';
        document.getElementById('btn-save-roster').onmouseout = (e) => e.target.style.background = '#d51a88';

        // Force live JS values into static HTML attributes before scraping so HP/Stats save
        const prepareDOMForSnapshot = () => {
            ...
            document.querySelectorAll('select').forEach(select => {
                ...
                select.querySelectorAll('option').forEach(option => {
                    if (option.value === val) option.setAttribute('selected', 'selected');
                    else option.removeAttribute('selected');
                });
            });
        };

        document.getElementById('btn-cache-view').onclick = () => {
            const activeTab = document.querySelector('.layout-tabbed-panel__tab-link.active');
            const label = activeTab && activeTab.innerText ? activeTab.innerText.trim() : `View ${capturedViews.length + 1}`;

            panel.style.display = 'none';
            prepareDOMForSnapshot();
            const html = document.documentElement.outerHTML;
            panel.style.display = 'flex';

            capturedViews.push({ label, html });
            document.getElementById('cache-count').innerText = `Views: ${capturedViews.length}`;
        };

        document.getElementById('btn-save-roster').onclick = () => {
            panel.style.display = 'none';
            let name = document.title.replace(' | Roll20', '').trim();
            if (!name) name = "Unnamed Character";

            if (capturedViews.length === 0) {
                prepareDOMForSnapshot();
                capturedViews.push({ label: 'Main', html: document.documentElement.outerHTML });
            }

            if (window.opener) {
                window.opener.postMessage({ type: 'ddb-sheet-save', name: name, views: capturedViews }, '*');
                window.close();
            } else {
                alert("Main Roll20 window not found.");
            }
        };
    }

    // =================================================================
    // MAIN ROSTER TOOL LOGIC
    // =================================================================
    function initMainTool() {
        ...
        let cachedSheets = GM_getValue('ddb_cached_sheets', {});

        let sortMode = 0;
        const sortLabels = ['Sort ⬇', 'Sort ⬆'];
        ...
        const normalizeName = (name) => name.trim().toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/gi, '');
        ...
        style.innerHTML = `
            ...
            #ddb-compact-stack { display: flex; flex-direction: column-reverse; gap: 5px; width: 100%; align-items: stretch; }
            .compact-btn { background: #333; color: #aaa; border: 1px solid #555; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 11px; font-weight: bold; transition: 0.2s; white-space: nowrap; text-align: center; box-shadow: 0 2px 5px rgba(0,0,0,0.3); width: 100%; box-sizing: border-box; }
            .compact-btn:hover { background: #444; color: #ffffff; border-color: #777; box-shadow: 0 0 8px rgba(255,255,255,0.1); }
            .compact-btn.cached { border-left: 3px solid #d51a88; color: #ff80cc; }
            .compact-btn.cached:hover { color: white; background: #d51a88; }
            ...
            #ddb-sub-header { display:none; background:#000; padding:6px; justify-content:center; gap:8px; border-bottom:1px solid #333; }
            .view-switch-btn { background: #222; color: #ccc; border: 1px solid #d51a88; border-radius: 4px; padding: 4px 12px; font-size: 11px; cursor: pointer; font-weight: bold; transition: 0.2s; white-space: nowrap; }
            .view-switch-btn.active { background: #d51a88; color: white; }
            .view-switch-btn:hover:not(.active) { background: #444; color: #fff; }
            ...
            #ddb-tracker { position: fixed; top: 100px; left: 100px; width: 220px; height: 350px; background: #1a1a1a; border: 1px solid #333; border-radius: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.7); z-index: 100003; display: none; flex-direction: column; overflow: hidden; font-family: 'Segoe UI', sans-serif; min-width: 200px; min-height: 250px; }
            ...
            .tracker-controls { background: #222; padding: 6px 10px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; }
            ...
            .tracker-val { font-weight: bold; border: 1px solid #444; border-radius: 4px; padding: 2px 6px; min-width: 20px; text-align: center; cursor: text; background: rgba(0,0,0,0.3); margin-left: auto; width: 44px; box-sizing: border-box; }
            ...
            .tracker-sort-btn { background: none; border: 1px solid #444; color: #aaa; border-radius: 4px; cursor: pointer; padding: 2px 0; width: 44px; text-align: center; font-size: 10px; font-weight: bold; transition: 0.2s;}
            .tracker-sort-btn:hover { background: #333; color: white; }

            #ddb-tracker-settings { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: #1a1a1a; z-index: 10; display: none; flex-direction: column; padding: 10px; box-sizing: border-box; }
            ...
            <div id="ddb-expansion-container">
                <div id="ddb-menu">
                    ...
                    <div id="ddb-settings-section">
                        ...
                        <div class="ddb-setting-row"><span>Auto-Track Chat</span><input type="checkbox" class="ddb-checkbox" id="set-chat-track" title="Automatically add rolls from chat to tracker"></div>
        ...
        frame.innerHTML = `
            ...
            <div id="ddb-sub-header"></div>
            <div id="ddb-shield"></div>
            <div id="ddb-iframe-container">
                <div id="ddb-cached-badge" style="display:none; position:absolute; bottom:15px; left:15px; background:#be1e2d; color:white; padding:4px 8px; font-size:12px; font-weight:bold; border-radius:4px; z-index:40; pointer-events:none; box-shadow:0 2px 5px rgba(0,0,0,0.5);">CACHED SHEET</div>
            </div>
        ...
        tracker.innerHTML = `
            ...
            <div class="tracker-controls">
                <button id="tracker-clear-btn" class="tracker-sort-btn" style="color:#ff4d4d; border-color:#8c1616; padding:2px 8px; width:auto;" title="Clear All">Clear</button>
        ...
        window.addEventListener("message", (event) => {
            if (event.data && event.data.type === 'ddb-sheet-save') {
                const { name, views } = event.data;
                const currentCached = GM_getValue('ddb_cached_sheets', {});
                currentCached[name] = views;
                GM_setValue('ddb_cached_sheets', currentCached);

                let currentChars = GM_getValue('ddb_chars_v25', []);
                const newChars = currentChars.filter(c => normalizeName(c.name) !== normalizeName(name));
                if(newChars.length !== currentChars.length) {
                    GM_setValue('ddb_chars_v25', newChars);
                }

                characterData = newChars;
                cachedSheets = currentCached;
                renderRoster();
                updateTabs(null);
            }
        }, false);
        ...
        const updateTrackerFromChat = (name, val) => {
            if(!name || !val) return;
            const normSearch = normalizeName(name);
            const existing = trackerData.find(t => normalizeName(t.name) === normSearch);
            if(existing) {
                existing.init = val;
            } else {
                trackerData.push({ name: name.trim(), init: val });
            }
            saveTracker();
        };
        ...
        document.getElementById('ts-add-party-btn').onclick = () => {
            characterData.forEach(char => {
                const normSearch = normalizeName(char.name);
                if(!trackerData.find(t => normalizeName(t.name) === normSearch)) {
                    trackerData.push({ name: char.name.trim(), init: 0 });
                }
            });
            const currentCached = GM_getValue('ddb_cached_sheets', {});
            Object.keys(currentCached).forEach(name => {
                 const normSearch = normalizeName(name);
                 if(!trackerData.find(t => normalizeName(t.name) === normSearch)) {
                     trackerData.push({ name: name.trim(), init: 0 });
                 }
            });
            saveTracker();
        };
        ...
        if(!chatLog) {
            setTimeout(startChatListener, 1000);
            return;
        }

        let chatReady = false;
        setTimeout(() => { chatReady = true; }, 15000);

        const obs = new MutationObserver(mutations => {
            if(!userSettings.autoTrack || !chatReady) return;
                ...
                m.addedNodes.forEach(node => {
                    if(node.nodeType === 1 && node.classList.contains('message')) {
                        ...
                        if(/initiative/i.test(text)) {
                            let val = 0;
                            const basic = node.querySelector('.basicdiceroll');
                            const inline = node.querySelector('.inlinerollresult');
                            const dieTotal = node.querySelector('.die__total');

                            if(basic) val = parseInt(basic.innerText);
                            else if(inline) val = parseInt(inline.innerText);
                            else if(dieTotal) val = parseInt(dieTotal.getAttribute('data-result') || dieTotal.innerText);

                            if(val) {
                                let name = '';

                                const metaName = node.querySelector('.meta__character-name');
                                if(metaName) name = metaName.innerText;

                                if(!name) {
                                    const captionName = node.querySelector('.sheet-rolltemplate-default caption');
                                    if(captionName) name = captionName.innerText;
                                }

                                if(!name) {
                                    const templateHeader = node.querySelector('.sheet-header .sheet-name');
                                    if(templateHeader) name = templateHeader.innerText;
                                }

                                if(!name) {
                                    const genericName = node.querySelector('strong, b, .sheet-charname');
                                    if(genericName) name = genericName.innerText;
                                }

                                if(!name) {
                                    const sheetName = node.querySelector('.sheet-name');
                                    if(sheetName) name = sheetName.innerText;
                                }

                                if(!name) {
                                    const by = node.querySelector('.by');
                                    if(by) name = by.innerText.replace(/:$/, '').trim();
                                }

                                if(name) {
                                    updateTrackerFromChat(name.trim(), val);
                                }
                            }
                        }
                    }
        ...
        const renderCachedView = (iframe, html) => {
        ...
        const openSheet = (id, type) => {
            ...
            const badge = document.getElementById('ddb-cached-badge');
            const switcher = document.getElementById('ddb-sub-header');
            ...
            if (id.startsWith('cached-')) {
                ...
                const charName = id.replace('cached-', '');
                const currentCached = GM_getValue('ddb_cached_sheets', {});
                const data = currentCached[charName];

                const views = Array.isArray(data) ? data : [{ label: 'Main', html: data }];
                ...
                views.forEach((v, idx) => {
                    ...
                    btn.onclick = () => {
                        ...
                        renderCachedView(iframe, v.html);
                    };
                    switcher.appendChild(btn);
                });

                renderCachedView(iframe, views[0].html);
        ...
        const updateTabs = (activeId) => {
            ...
            if (!activeId) {
                document.getElementById('ddb-sub-header').style.display = 'none';
                document.getElementById('ddb-cached-badge').style.display = 'none';
            }
            ...
            Object.keys(currentCached).forEach(name => {
                ...
                const tab = document.createElement('div');
                tab.className = `ddb-tab ${id === activeId ? 'active' : ''}`;
                tab.style.backgroundColor = '#d51a88';
                tab.innerText = name;

                const closeBtn = document.createElement('span');
                closeBtn.className = 'ddb-tab-close';
                closeBtn.innerHTML = '&times;';
                closeBtn.onclick = (e) => {
                    e.stopPropagation();
                    delete currentCached[name];
                    GM_setValue('ddb_cached_sheets', currentCached);
                ...
                tab.onclick = () => openSheet(id, 'cached');
                tabBar.appendChild(tab);
            });
        ...
        const renderRoster = () => {
            ...
            Object.keys(currentCached).forEach(name => {
                ...
                item.className = 'char-item';
                item.style = "display: flex; align-items: center; justify-content: space-between; padding: 10px 5px; border-bottom: 1px solid #333; cursor: pointer;";
                item.innerHTML = `<span style="flex-grow:1; font-size: 13px; color: #ff80cc;">[Cached] ${name}</span><div style="display:flex; gap:12px;"><span style="color:#ff4d4d; font-size:11px; font-weight:bold;" data-delcache="${name}">DEL</span></div>`;
                item.onclick = (e) => {
                    if (e.target.dataset.delcache) {
                        if(confirm(`Delete cached sheet ${name}?`)) {
                            delete currentCached[name];
                            GM_setValue('ddb_cached_sheets', currentCached);
                            const ifr = document.getElementById(`iframe-cached-${name}`);
                            if(ifr) ifr.remove();
                            renderRoster();
                            updateTabs(null);
                        }
                    }
                    else openSheet('cached-' + name, 'cached');
                };
                list.appendChild(item);

                const cBtn = document.createElement('div');
                cBtn.className = 'compact-btn cached';
                cBtn.innerText = name;
                cBtn.onclick = () => openSheet('cached-' + name, 'cached');
                stack.appendChild(cBtn);
            });
        };
        ...
        const updateVisibility = () => {
            ...
            document.getElementById('set-chat-track').checked = userSettings.autoTrack;
        ...
        const saveSettings = () => {
            ...
            userSettings.autoTrack = document.getElementById('set-chat-track').checked;
        ...
        ['set-notepad', 'set-tracker', 'set-chat-track', 'set-notebook', 'set-5e', 'set-newtab'].forEach(id => document.getElementById(id).onchange = saveSettings);
